document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    const response = await fetch('/logs/api/metrics');
    ...
    data.slowEndpoints.forEach(item => {
      ...
      row.innerHTML = `
        ...
        <td class="px-4 py-2 border">${(item.avgTime / 1000).toFixed(3)} s</td>
  ...
  async function loadSlowEndpoints(windowKey = '1d') {
    ...
    try {
      ...
      slowList.innerHTML = '<li class="text-red-500">Error loading data</li>';
    }
  }
